...
class _StartQuizScreenState extends State<StartQuizScreen> {
  ...
  void nextItem() {
    ...
    scrollController.animateTo(index * 377.0.w,
              ...
              child: ListView.builder(
                  ...
                  itemBuilder: (context, int index) {
                    return StartQuizWidget(
                      questionModel: quizProvider.questions[index],
                    );
                  }),
            ),
            Padding(
                      ...
                      onPressed: index == quizProvider.questions.length - 1
                          ? () {
                              if (quizProvider.selectedAnswer ==
                                  quizProvider.questions[index].correctAnswer) {
                                quizProvider.score++;
                                log(quizProvider.score.toString());
                              }
                              quizProvider.changeResultStatus();
                              Navigator.of(context)
                                  .pushReplacementNamed('/resultScreen');
                            }
                          : null,
                      child: Text(
                        'Finish',
